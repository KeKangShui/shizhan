        ...
        if re.match('http://www.xiaohuar.com/list-1-\d+.html', response.url):#如果url能够匹配到需要爬取的url，就爬取
            ...
            for i in range(len(items)):#遍历div个数
                ...
                if src:
                    ...
                    file_path = os.path.join("D:\\1\\pic\\bib\\pic", file_name)#拼接这个图片的路径，我是放在F盘的pics文件夹下
                    urllib.request.urlretrieve(absoluteSrc, file_path)#接收文件路径和需要保存的路径，会自动去文件路径下载并保存到我们指定的本地路径
